...
- No Android network permission in the current foundation; emoji recents, emoji search, suggestions, alternates, portable preferences, and the 0.1.22 Draft crash-recovery Clipboard surface do not synchronize or emit telemetry.
- Draft PR #97 / 0.1.22-dev is the active crash-recovery candidate after 0.1.21 was rejected as representative-device evidence because the launcher/setup app could stop immediately on launch. 0.1.22 adds forced first-run activity-launch instrumentation, an Android 12/API 31 launcher + IME runtime lane, and fail-closed enabled/default IME status reads in addition to the Android 15 runtime lane.
- Draft PR #97 / 0.1.22-dev is the recovery candidate after 0.1.21 was rejected on representative hardware for an immediate launcher/setup crash. It adds a forced first-run Activity launch regression, an Android 12/API 31 launcher + IME lane, and fail-closed enabled/default IME status reads in addition to the Android 15 runtime lane.
...
- Draft PR #97 / 0.1.22-dev extends the first-party Clipboard toolbar action and Keyboard-side panel with tap-to-paste, Paste Once, dedicated Pinned and Recent collections, pin/unpin, long-press management, encrypted saved-clip editing, delete, clear-unpinned, bounded expiration, per-app Allow/Ask/Paste-only/Block policy, Android-sensitive non-persistence, and local smart-content extraction for phone numbers, email addresses, web links, street addresses, dates, and times. Derived fragments are not separately persisted or used for language learning.
