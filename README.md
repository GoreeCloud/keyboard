...
- The current 0.1.13 Development candidate uses the version-scoped package `com.goreecloud.keyboard.dev.v14` and visible name **GoreeCloud Keyboard Dev 0.1.13**, so it installs alongside a preinstalled `com.goreecloud.keyboard` package and earlier Development builds instead of attempting an incompatible signature update.
