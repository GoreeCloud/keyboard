...
- The current 0.1.14 Development candidate uses the version-scoped package `com.goreecloud.keyboard.dev.v15` and visible name **GoreeCloud Keyboard Dev 0.1.14**, so it installs alongside a preinstalled `com.goreecloud.keyboard` package and earlier Development builds instead of attempting an incompatible signature update.
...
- A built-in read-only GoreeCloud dictionary that keeps the first-party frequency-ordered Quill lexicon as primary ranking authority and adds a 46,855-entry lowercase English fallback derived from the public-domain Moby Words II common-word list. The fallback is packaged locally and covers common words missing from the earlier candidate, including `jump`, `lag`, and `lagging`. Optional personalization is isolated from this built-in dictionary and is disabled by default.
