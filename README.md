...
- A bounded local swipe-typing Development path using transient QWERTY gesture geometry and packaged-dictionary ranking rather than literal crossed-key text, with a visible gesture trail, a deliberate minimum travel/time/path threshold to reduce accidental swipes during fast tapping, and sensitive-editor/touch-exploration suppression.
- The current 0.1.6 Development candidate uses the version-scoped package `com.goreecloud.keyboard.dev.v7` and visible name **GoreeCloud Keyboard Dev 0.1.6**, so it installs alongside a preinstalled `com.goreecloud.keyboard` package and earlier Development builds instead of attempting an incompatible signature update.
- A launcher-visible **GoreeCloud Keyboard** settings application plus an in-keyboard Settings control. Device-local settings currently cover swipe typing, suggestions, autocorrect, next-word predictions, automatic capitalization, and three visible key-height choices: Compact, Standard, and Tall. Full touch targets remain preserved behind the visual keycap sizes.
- A built-in read-only GoreeCloud dictionary derived from the canonical branding catalog, including canonical product/system names and common English contractions. It does not learn from typed text.
- A bounded local Quill prediction/grammar layer that can present up to three next-word predictions from the current transient editor session, prioritize likely spelling corrections in the candidate strip, apply conservative boundary autocorrection, restore common apostrophes, preserve canonical GoreeCloud casing, and automatically capitalize sentence starts without persistent language-history storage.
