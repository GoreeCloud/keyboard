...
- The current 0.1.10 Development candidate uses the version-scoped package `com.goreecloud.keyboard.dev.v11` and visible name **GoreeCloud Keyboard Dev 0.1.10**, so it installs alongside a preinstalled `com.goreecloud.keyboard` package and earlier Development builds instead of attempting an incompatible signature update.
