...
- A bounded local swipe-typing Development path using transient QWERTY gesture geometry and packaged-dictionary ranking rather than literal crossed-key text, with a visible gesture trail, a deliberate minimum travel/time/path threshold to reduce accidental swipes during fast tapping, tolerant-but-bounded endpoint drift for more realistic gestures, context-aware re-ranking of already geometry-approved candidates, post-swipe correction alternatives, and sensitive-editor/touch-exploration suppression.
- The current 0.1.8 Development candidate uses the version-scoped package `com.goreecloud.keyboard.dev.v9` and visible name **GoreeCloud Keyboard Dev 0.1.8**, so it installs alongside a preinstalled `com.goreecloud.keyboard` package and earlier Development builds instead of attempting an incompatible signature update.
- A launcher-visible **GoreeCloud Keyboard** settings application with a Glaze card-based hierarchy plus an in-keyboard Settings control. Device-local settings currently cover swipe typing, suggestions, autocorrect, next-word predictions, automatic capitalization, key-press vibration, an off-by-default **Learn from what you type** control, toolbar style, and three visible key-height choices: Compact, Standard, and Tall. Full touch targets remain preserved behind the visual keycap sizes.
...
- A bounded local Quill prediction/grammar layer that presents three starter predictions before typing, provides broader common-context continuations, can present up to three next-word predictions from the current transient editor session, prioritizes likely spelling corrections in the candidate strip, applies conservative boundary autocorrection, restores common apostrophes, preserves canonical GoreeCloud casing, and automatically capitalizes sentence starts. When the user explicitly enables learning, Keyboard adds bounded device-local word/bigram counters to candidate ranking; sensitive and host no-suggestions editors are excluded and learned data can be cleared from Settings.
- A dedicated real-action utility toolbar above the number row exposes **Emoji, Settings, and Hide Keyboard**. The default style is icons-only, with an optional Icons + labels mode. Symbols remain on the bottom-row ?123/ABC controls rather than being duplicated in the toolbar; Emoji is toolbar-only on letter/symbol layers. Clipboard and GIF controls remain intentionally absent until their governed providers/authority are real.
- Backspace uses bounded Unicode-aware deletion when editor context is available and falls back to the standard Android DEL key event when an editor does not support the preferred deletion API, improving compatibility without expanding text-read authority. Backspace and Enter also use first-party vector-style key glyphs rather than relying on generic text symbols.
