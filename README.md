...
- The current 0.1.11 Development candidate uses the version-scoped package `com.goreecloud.keyboard.dev.v12` and visible name **GoreeCloud Keyboard Dev 0.1.11**, so it installs alongside a preinstalled `com.goreecloud.keyboard` package and earlier Development builds instead of attempting an incompatible signature update.
