...
The Clipboard action now has a real Keyboard-side implementation rather than a placeholder: deliberate paste, Paste Once, opt-in encrypted local history, pin/unpin, delete, clear-unpinned, 10-minute/1-hour/24-hour retention, Android-sensitive non-persistence, and current-app Allow/Ask/Paste-only/Block policy. GIF/media insertion remains future work and still requires an explicit provider, network/retention behavior, search/content-safety boundary, user controls, and privacy/security acceptance.
...
| FR-017 | Partial Draft candidate implementation: Keyboard-side intentional paste, per-app local policy, Paste Once, and separately governed encrypted history exist in PR #97; privileged Secure Paste Broker, Privacy Shield authorization, Wardveil acceptance, and system-wide enforcement remain blocked. |
